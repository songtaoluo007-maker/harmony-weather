...
[原生前后对比](design/atmosphere-comparison.jpg) · [范围与验证记录](docs/atmosphere-polish.md)。
保留城市景观和真实数据，仅调整空气质量、生活气象、天气详情及相邻背景层。
本轮 59 项测试通过；未签名 HAP 构建、安装、启动及原生点击详情通过。
新增 320vp 父容器 / 1.3× 字体组合检查，修正卡片不等高和单位拥挤；
恢复正常约 377vp 界面后再次截图。所有临时宽度与字体钩子已移除。
这是限定区域的原生检查，不代表全部尺寸、日照状态或读屏服务已经验收。
